...
const WHATSAPP_NUMBER = "+5491121826396";
...
export async function fetchProductsFromSheets() {
  ...
  const data = (parsed.data || [])
    .map((r, idx) => {
      const nombre = findColumn(r, "Producto")?.trim();
      if (!nombre) return null;

      const stock = parseNumberLoose(findColumn(r, "Stock"));
      const precioRaw = findColumn(r, "Precio Minorista");
      const precio = parseNumberLoose(precioRaw);
      const categoria = (findColumn(r, "Categoria") || "Sin categoría").trim();
      const imagenRaw = findColumn(r, "ImagenURL");
      const imagen = imagenRaw?.trim();
      const descripcion = findColumn(r, "Descripcion")?.trim();

      // Debug del primer producto
      if (idx === 0) {
        console.log("Primer producto parseado:", {
          nombre,
          precioRaw,
          precio,
          imagenRaw,
      ...
      return {
        id: `${nombre}-${idx}`.replace(/\s+/g, "-").toLowerCase(),
        nombre,
        stock,
        precio,
        categoria,
  ...
  console.log("Productos parseados:", data.length);
  return data;
}
